// /studio/intents/:intentId — autoresearch detail panel for the
// scientist intent (T13). Match the LumidOS mockup:
//   - intent meta + h1
//   - stats row (4 tiles)
//   - Pareto chart (accuracy vs latency, 200ms budget line)
//   - direction badge
//   - variants table v3a–v3d
// SVG is hand-rolled to avoid pulling in a chart library — the mockup
// dictates the exact layout, and a 12-dot scatter is trivial.
...
interface IntentDetail {
	...
	stats: Stat[];
	variants: Variant[];
	// Chart coordinates from the mockup's SVG (chart pixel space).
	// Frontier points are connected; others are scattered.
	chart: {
		frontier: Array<{ cx: number; cy: number }>;
		underBudget: Array<{ cx: number; cy: number }>;
		overBudget: Array<{ cx: number; cy: number }>;
	};
}

const DETAILS: Record<string, IntentDetail> = {
	scientist: {
		persona: 'scientist · cycle 3 · auto-sysresearch',
		title: 'Find the best NL-to-SQL config under 200ms',
		cycleOf: 'cycle 3 of 12',
...
export default function StudioAutoresearch() {
	const { intentId = '' } = useParams<{ intentId: string }>();
	const detail = DETAILS[intentId];

	if (!detail) {
		return (
			<div className="max-w-3xl mx-auto py-12 text-center">
				<div className="text-sm text-slate-500 mb-3">No autoresearch detail for this intent.</div>
	...
	return (
		<div className="max-w-4xl mx-auto px-1 py-2 space-y-5">
			...
			<header className="flex items-end justify-between gap-3">
				<div>
					<div className="text-[11px] text-slate-400 mb-1">{detail.persona}</div>
					<h1 className="text-lg font-medium text-slate-900 leading-tight">{detail.title}</h1>
				</div>
				<div className="text-[11px] text-slate-400 flex-shrink-0">{detail.cycleOf}</div>
			</header>
			...
			<section className="rounded-xl border border-slate-200/70 bg-white px-4 py-3.5">
				...
				<ParetoChart chart={detail.chart} />
			</section>
					...
					<table className="w-full text-[12px]">
						...
						<tbody>
							{detail.variants.map((v, i) => (
								<tr key={v.id} className={i < detail.variants.length - 1 ? 'border-b border-slate-200/70' : ''}>
...
function ParetoChart({ chart }: { chart: IntentDetail['chart'] }) {
	// Coordinates lifted from the mockup SVG; chart viewport 0..600×240.
