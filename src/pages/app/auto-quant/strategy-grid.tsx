...
function Sparkline({ sharpe }: { sharpe?: number }) {
	...
	return (
		<svg width={w} height={h} className="overflow-visible">
			<path d={d} fill="none" stroke={isPositive ? "#16a34a" : "#ef4444"} strokeWidth={1.5} />
		</svg>
	);
}
				...
				return (
					...
					>
						...
						<div className="flex items-start justify-between gap-2">
							...
							{pnl != null && (
								<div className={`text-xs font-medium ${pnl >= 0 ? "text-green-600" : "text-red-500"} flex items-center gap-0.5 mt-0.5`}>
							...
							<div className="text-right text-[10px] text-muted-foreground space-y-0.5">
								{s.recent_sharpe != null && (
									<div>
										...
										<span className={`font-mono font-medium ${s.recent_sharpe >= 1 ? "text-green-600" : s.recent_sharpe >= 0 ? "text-gray-600" : "text-red-500"}`}>
