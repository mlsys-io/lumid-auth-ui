					...
					{strategy && (
						<div className="mt-1 flex items-center gap-3 text-sm text-muted-foreground">
							...
							{strategy.recent_sharpe != null && (
								<span>Sharpe: <strong className={strategy.recent_sharpe >= 1 ? "text-green-600" : strategy.recent_sharpe >= 0 ? "text-gray-700" : "text-red-500"}>{strategy.recent_sharpe.toFixed(2)}</strong></span>
							)}
							{strategy.lifetime_pnl != null && (
								<span>Lifetime P&L: <strong className={strategy.lifetime_pnl >= 0 ? "text-green-600" : "text-red-500"}>{strategy.lifetime_pnl >= 0 ? "+" : ""}{strategy.lifetime_pnl.toFixed(2)}pp</strong></span>
							)}
						</div>
					)}
				</div>
							...
							return (
								<div key={stage} className="flex items-center">
									...
									{i < LIFECYCLE_ORDER.length - 1 && (
										<span className={`mx-0.5 text-gray-300 text-[10px] ${isPast ? "text-green-400" : isFuture ? "" : ""}`}>→</span>
									)}
								</div>
							);
						})}
