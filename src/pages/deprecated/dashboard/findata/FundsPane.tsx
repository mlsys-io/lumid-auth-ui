...
type Sub = "ownership" | "disclosure";

export default function FundsPane({ symbol }: { symbol: string }) {
  const [sub, setSub] = useState<Sub>("ownership");
  ...
  useEffect(() => {
    ...
    const p = sub === "ownership"
      ? findata.fundOwnership(symbol)
      : findata.fundsDisclosure(symbol);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {!loading && !err && rows.length === 0 && <p className="text-sm text-muted-foreground">No data.</p>}
