...
export default function LearningTimeline({ agents }: { agents: string[] }) {
	...
	return (
		<div className="space-y-2">
			...
			<ul className="space-y-1">
				{data.recent.map((m, i) => (
					<li key={m.id || i} className="flex items-start gap-1.5 text-[11px]">
						<span className={`mt-1 w-1.5 h-1.5 rounded-full flex-shrink-0 ${(m.kind || "").match(/correction|anti/) ? "bg-amber-400" : "bg-amber-400"}`} />
			...
			<Link to="/studio/knowledge" className="inline-flex items-center gap-1 text-[11px] text-slate-500 hover:text-amber-700 transition-colors">
