			...
			<section className="rounded-xl border border-slate-200 bg-white p-4 space-y-2">
				...
				<div className="flex gap-2">
					<input
						...
						className="flex-1 px-2.5 py-1.5 text-[13px] rounded-lg border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-400/20 focus:border-emerald-400 disabled:bg-slate-50"
					/>
					<button
						onClick={saveRename}
			...
			<section className="rounded-xl border border-slate-200 bg-white p-4 space-y-2.5">
				<div className="flex items-center gap-1.5 text-[13px] font-medium text-slate-800">
					...
					<button
						...
						disabled={readOnly}
						className="ml-auto inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[12px] font-medium bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-40 transition-all"
					>
						...
						{filtered.map((s) => {
							...
							return (
								<div key={s.slug} className="flex items-center gap-3 px-3 py-2">
									...
									{already ? (
										<span className="text-[11px] text-emerald-600 inline-flex items-center gap-1 flex-shrink-0"><Check className="w-3 h-3" /> imported</span>
									) : (
										<button
											onClick={() => add(s)}
	...
	return (
		<Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
			<DialogContent className="max-w-md">
				<DialogHeader>
					<DialogTitle className="flex items-center gap-2 text-[15px]">
						<CalendarClock className="w-4 h-4 text-emerald-500" /> New workflow
					</DialogTitle>
					<DialogDescription className="text-[12.5px]">
				...
				<div className="space-y-3">
					<label className="block">
						...
						<input
							...
							className="w-full px-2.5 py-1.5 text-[13px] rounded-lg border border-slate-200 font-mono focus:outline-none focus:ring-2 focus:ring-emerald-400/20 focus:border-emerald-400"
						/>
					...
					<label className="block">
						...
						{!customCron ? (
							...
							<input
								...
								className="w-full px-2.5 py-1.5 text-[13px] rounded-lg border border-slate-200 font-mono focus:outline-none focus:ring-2 focus:ring-emerald-400/20"
							/>
						)}
					</label>
					<label className="block">
						...
						<input
							...
							className="w-full px-2.5 py-1.5 text-[12.5px] rounded-lg border border-slate-200 font-mono focus:outline-none focus:ring-2 focus:ring-emerald-400/20"
						/>
					...
					<label className="block">
						...
						<input
							...
							className="w-full px-2.5 py-1.5 text-[13px] rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-400/20"
						/>
					</label>
					<button
						onClick={() => onCreate({
						...
						disabled={!valid}
						className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-40 transition-all"
					>
			...
			<div className="grid gap-2.5 sm:grid-cols-3">
				<div className="rounded-lg border border-slate-200/80 p-3 space-y-1.5">
					...
					<input value={forkName} onChange={(e) => setForkName(e.target.value)}
						className="w-full px-2 py-1 text-[11px] font-mono rounded border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-400/30" />
				...
				<div className="rounded-lg border border-slate-200/80 p-3 space-y-1.5">
					...
					<button disabled={!!busy || readOnly} onClick={() => post("publish", "publish", {},
						...
						className="w-full inline-flex items-center justify-center gap-1 px-2 py-1.5 text-[12px] rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-40">
