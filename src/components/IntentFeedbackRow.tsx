	...
	if (state === 'good' || state === 'wrong' || state === 'edited') {
		const label =
			...
			state === 'wrong'  ? 'noted · adjusting' :
			                     'noted';
		const tone =
			state === 'good'  ? 'text-emerald-700 bg-emerald-50 border-emerald-100' :
		...
		return (
			<div className="flex items-center gap-1">
				<input
					...
					className="px-2 py-1 text-[12px] rounded-md border border-slate-200 focus:border-emerald-300 focus:ring-1 focus:ring-emerald-200 outline-none w-[200px]"
					autoFocus
				/>
				<button
