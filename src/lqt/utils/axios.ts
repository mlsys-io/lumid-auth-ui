...
/**
 * GET that forwards the viewer's scoped session-bearer as `Authorization`.
 *
 * For super_admin-gated same-origin endpoints — notably the opsagent
 * `/lqt/strategies-roster`, which introspects the CALLER's role (`role ==
 * super_admin`) rather than accepting the read-scoped service PAT the landing
 * nginx injects for the plain read paths. The nginx `/lqt/` location uses
 * caller-bearer-wins precedence (it injects a PAT only when the caller sends
 * none), so a forwarded bearer reaches the opsagent unchanged. The bearer is
 * minted from the browser's `lm_session` cookie via `/api/v1/session-bearer`
 * (`getBearer`), so a super_admin viewer authenticates as themselves and no
 * longer depends on a separately-rotated service PAT for that path. Single-shot
 * 401 retry with a fresh token (a stale cached bearer self-heals).
 *
 * The plain `getJson` (no Authorization) stays the right helper for the
 * unauthenticated read paths — do not route those through here or the injected
 * read PAT would be overridden.
 */
export async function getJsonAuthed<T>(
  path: string,
  params?: Record<string, unknown>,
): Promise<T> {
  const attempt = async (): Promise<T> => {
    const { token } = await getBearer();
    const res = await lqtAxios.get<T>(path, {
      params,
  ...
  try {
    ...
    if (e instanceof LqtGatewayError && e.status === 401) {
